function assimilation( gens, size, inputMode )
	...
	while gens > 0
		...
		for y = 1:size
			for x = 1:size
				...
				% rules
				if cells(x, y) == 1 && sumN < 4	% underpopulation
					nextGen(x, y) = 0;
				end
				if cells(x, y) == 1 && (sumN >=4 && sumN <= 7)	% statis
					nextGen(x, y) = 1;
				end
				if cells(x, y) == 1 && sumN > 7	% overpopulation
					nextGen(x, y) = 0;
				end
				if cells(x, y) == 0 && (sumN == 3 || sumN == 4 || sumN == 5)	% reproduction
					nextGen(x, y) = 1;
				end
