function maze( gens, size, inputMode )
	...
	while gens > 0
		...
		for y = 1:size
			for x = 1:size
				...
				% rules
				if cells(x, y) == 1 && sumN == 0	% underpopulation
					nextGen(x, y) = 0;
				end
				if cells(x, y) == 1 && (sumN >=1 && sumN <= 5)	% statis
					nextGen(x, y) = 1;
				end
				if cells(x, y) == 1 && sumN > 5	% overpopulation
					nextGen(x, y) = 0;
				end
				if cells(x, y) == 0 && sumN == 3	% reproduction
					nextGen(x, y) = 1;
				end
